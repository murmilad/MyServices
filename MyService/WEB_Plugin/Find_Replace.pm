package MyService::WEB_Plugin::Find_Replace;
...
use constant TASK_MAP => {
	'example' => {
		...
		parameters => {
			tables => {
				TABLE               => {all_fields => 1, pkg_name => 'ttt'},
			}
		},
		check_str => sub {
			my %h = @_;	
...
sub update_file {
	...
	if (open INPUT_FILE, "<$file_path"){
		if (open OUTPUT_FILE, ">$file_path.tmp"){
			...
			$self->print_html_log("Error: Can't open $file_path");
		}
		close INPUT_FILE;
	} else {
		print_log("Error: Can't open $file_path");
...
sub get_files_list {
	...
	if ($svn_replase) {
		...
		my $rm_result = `$rm_cmd`;
		print_log($rm_result);
		$self->print_html_log($rm_result);
		

		print_log("get_svn_trunks");
		$self->print_html_log("get_svn_trunks");
...
#		map {$_ =~ s/\/Module//} @{$trunk_paths};
		...
		$self->print_html_log(Dumper($trunk_paths));
		...
		foreach my $trunk_path (@{$trunk_paths}) {
			...
			print_log("cd $svn_modules_path/Module/$module_path; svn checkout $recursive $trunk_path");
			$self->print_html_log("cd $svn_modules_path/Module/$module_path; svn checkout $recursive $trunk_path");

			$self->exec_cvn_command("cd $svn_modules_path/Module/$module_path; svn checkout $recursive $trunk_path <login> ");

			if ($trunk_path =~ /Branch/){
				print_log("cd $svn_modules_path/Module/$module_path/Branch; svn checkout $trunk_path/$branch_name");
				$self->print_html_log("cd $svn_modules_path/Module/$module_path/Branch; svn checkout $trunk_path/$branch_name");
				$self->exec_cvn_command("cd $svn_modules_path/Module/$module_path/Branch; svn checkout $trunk_path/$branch_name <login> ");
			}
		...
		$self->exec_cvn_command('svn update ' . &SVN_MODULES_PATH . '/Module/ <login>');
	}
	...
	$self->print_html_log("Found: all_files_list = " . Dumper($all_files_list));
	print_log("Found: module_files_list = " . Dumper($module_files_list));
	$self->print_html_log("Found: module_files_list = " . Dumper($module_files_list));
	print_log("Found: absent_files_list = " . Dumper($absent_files_list));
	$self->print_html_log("Found: absent_files_list = " . Dumper($absent_files_list));
	return {
...
sub create_branches {
	...
	foreach my $file_name (keys %{$all_files_list}) {
		...
		$self->print_html_log("Change $source_path:\n\t$dest_path$name");
		unless (-f "$dest_path$name"){
			...
			unless (-d "$module_path/Branch/$branch_name/.svn"){
				$self->exec_cvn_command("cd $module_path/Branch; svn add $module_path/Branch/$branch_name");
			}
			...
			foreach my $dir (split('/', $file_path)){
				...
				unless (-d "$module_path/Branch/$branch_name$dir_path/.svn"){
					$self->exec_cvn_command("cd $module_path/Branch/$branch_name$current_path; svn add $module_path/Branch/$branch_name$dir_path");
				}
			}

			$self->exec_cvn_command("svn copy $source_path $dest_path$name <login> ");
			

			$self->exec_cvn_command("svn commit $module_path/Branch/$branch_name -m '$message (start version)' <login> ");
	...
	foreach my $file_name (keys %{$all_files_list}) {
		...
		my $result = $self->update_file(file_path => "$dest_path$name", task => $task, check_result => $check_result);		
		
		if ($result) {
			$self->exec_cvn_command("svn commit $module_path/Branch/$branch_name -m '$message' <login> ");
	...
	foreach my $file_name (keys %{$all_files_list}) {
		...
		$self->print_html_log(qq{Finded: $source_path});
	}	
	...
	foreach my $file_name (keys %{$absent_files_list}) {
		...
		$self->print_html_log(qq{Look: $source_path});
	}
...
sub exec_cvn_command {
	...
	$self->print_html_log("$russify $command");
	sleep 2;
	system ("$russify $command;");

	if ($? == -1) {
        print_log("failed to execute: $!");
        $self->print_html_log("failed to execute: $!");
    }
    elsif ($? & 127) {
        print_log(sprintf "child died with signal %d, %s coredump",
            ($? & 127),  ($? & 128) ? 'with' : 'without');
        $self->print_html_log(sprintf "child died with signal %d, %s coredump",
            ($? & 127),  ($? & 128) ? 'with' : 'without');
    }
    else {
        print_log(sprintf "child exited with value %d", $? >> 8);
        $self->print_html_log(sprintf "child exited with value %d", $? >> 8);
    }
...
sub find_replase {
	...
	if ($@) {
		...
		my $files_list = $self->get_files_list(
			task        => $task,
			svn_replase => 1,
#			svn_update  => 1,
		);
		...
		if ($files_list->{error}) {		
			$self->print_html(q{</div>});
			return {
				error => $files_list->{error}
			};
		}

		my $result = $self->create_branches(
			task => $task,
...
sub handler {
		...
		foreach my $find_req_path (@find_reqs) {
			if (open FIND_PARAMS_OBJ, "</var/Project/log/find_replace/$find_req_path") {
				...
				$self->print_html_log("$! /var/Project/log/find_replace/$find_req_path");
			}
		}
		if ($param_hash->{task}) {
			...
			my $result = $self->find_replase(
				task_command => $param_hash->{task},
				old_find_params => $old_find_params,
			);

            $self->print_html("<h1>Find/Replace result</h1>");
			if ($result->{error}) {
	            $self->print_html("Error: $result->{error}");
			} else {
                ...
                $self->print_html("<h1>Enter Find/Replace data</h1>");
                $self->print_html(qq|
					...
					<input type="hidden" name="handler" value="find_replace"/>
